...
class PrintNetwork:
    ...
    def __init__(self, file_path):
        ...
        self.arrange_nodes()
        self.layer_wise_nodes = [layer['nodes'] for layer in self.data['layers']]
        self.last_layer_length = len(self.data['layers'][-1]['nodes'])
    ...
    def print_network(self):
        ...
        for out_index, nodes_in_layer in enumerate(self.layer_wise_nodes):
            ...
            self._print_line(nodes_in_layer, connection_block_number,
                             lambda node: PrintColor.print_in_green_back(center_string(node["name"])), last_layer)
            self._print_line(nodes_in_layer, connection_block_number,
                             lambda node: PrintColor.print_in_green_back(center_string(node["ip"])))

            if not last_layer:
                self._print_line(nodes_in_layer, connection_block_number,
                                 lambda node: PrintColor.print_in_red(center_string("\u2191")))
                self._print_line(nodes_in_layer, connection_block_number,
                                 lambda node: PrintColor.print_in_red(center_string("|")))
                self._print_line(nodes_in_layer, connection_block_number,
                                 lambda node: PrintColor.print_in_red(center_string("\u2193")))
    ...
    def arrange_nodes(self):
        self.data['layers'] = sorted(self.data['layers'], key=lambda layer: len(layer['nodes']))
        for layer in self.data['layers']:
            layer['nodes'] = sorted(layer['nodes'], key=lambda node: node['node_id'])
